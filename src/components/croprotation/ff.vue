...
<template>
        ...
        <v-row>
          <v-col cols="3">
            <NumberInput
              ...
              style="width: 100%"
              hide-details="auto"
              @update:model-value="tab = 'auswahl'"
            ></NumberInput>
          ...
          <v-col>
            <v-tabs-window v-model="tab" style="height: 300px" class="w-100">
              <v-tabs-window-item value="schnitt">
                <schnitt_select
                  v-model:schnitte="FF[jahr].schnitt_menge"
                ></schnitt_select>
              </v-tabs-window-item>
              ...
              <v-tabs-window-item value="anbau" style="overflow-y: scroll">
                <v-card elevation="0">
                  <v-card-item>
                    <v-card-title> <l>ANBAU_OPTS</l> </v-card-title>
                  ...
                  <v-card-text>
                    <v-select
                      v-if="FF[jahr].vis.leg_ansaat"
                      style="height: 80px"
                      class="custom-label-color"
                      :label="L('Ansaat')"
                      :items="FF[jahr].leg_ansaat_opts.map((key) => ({
                          value: key,
                          title: L(key),
                        }))"
                    ...
                    <v-checkbox
                      :label="L('UNDERSAWN')"
                      v-if="FF[jahr].vis.us_opt"
                      v-model="FF[jahr].us"
                    ></v-checkbox>     
                    <v-checkbox
                      :label="L('Nebenprodukternte')"
                      v-if="FF[jahr].vis.stroh_opt"
                      v-model="FF[jahr].stroh"
                    ></v-checkbox>
                    <v-checkbox
                      :label="L('Zwischenfrucht_select')"
                      v-if="FF[jahr].vis.zw_opt"
                      v-model="FF[jahr].zw"
                    ></v-checkbox>

                    <v-select
                      style="height: 80px"
                      :label="L('Zwischefruchtanbau')"
                      :items="
                        FF[jahr].zw_plant_opts.map((key) => ({
                          value: key,
                          title: L(key),
                        }))
                      "
                      class="custom-label-color"
                      v-model="FF[jahr].zw_plant"
                      v-if="FF[jahr].zw"
                    >
                    </v-select>
                    ...
                    <NumberInput
                      v-if="FF[jahr].zw"
                      class="custom-label-color"
                      :label="L('LEG_SELECT_IN_ZW')"
                      suffix="% TS"
                      v-model="FF[jahr].zwischenfrucht_legant"
                    ></NumberInput>

                    <v-select
                      v-if="FF[jahr].zw"
                      class="custom-label-color"
                      :label="L('SEL_WINTER_HARD')"
                      :items="
                        ['abfrierend', 'frosthart'].map((key) => ({
                          value: key,
                          title: L(key),
                        }))
                      "
                      v-model="FF[jahr].zwischenfrucht_winterhard"
                    ></v-select>
                    ...
                    <v-select
                      ...
                      :label="L('SEL_ZW_NUTZ')"
                      :items="
                        ['Abfuhr', 'Einarbeitung'].map((key) => ({
                          value: key,
                          title: L(key),
                        }))
                      "
                      v-model="FF[jahr].zwischenfrucht_schnittnutz"
                    ></v-select>
                  </v-card-text>
                </v-card>
              </v-tabs-window-item>
              <v-tabs-window-item value="dung" style="overflow-y: scroll;overflow-x: clip;height: 100%;" >
                <dung_select
              ...
              <v-tabs-window-item value="auswahl">
                <v-card elevation="0">
                  ...
                  <v-card-text>
                    <v-checkbox
                      :label="L('RESTRICT_PHYTO')"
                      v-model="FF[jahr].restr_select_phyto"
                    ></v-checkbox>
                    <v-checkbox
                      :label="L('RESTRICT_TIME')"
                      v-model="FF[jahr].restr_select_time"
                    ></v-checkbox>
                  </v-card-text>
                </v-card>
              </v-tabs-window-item>
  ...
  <v-row no-gutters class="ma-0 pa-0" style="width: 100%">
    <v-col v-for="n in NJahre" :key="n" class="ma-0 pa-0">
      <ffcomp @click="jahr=n;tab='auswahl'"
        :jahr="n"
      ></FFComp>
    </v-col>
  </v-row>
